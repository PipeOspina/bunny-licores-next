...
export const useCharging = <T extends TCharging>(
	...
	const isGlobalCharging = Object.values(stateGlobalCharging || {}).includes(
		true,
	);
	...
	const setGlobalCharging = (
		item: keyof IGlobalCharging,
		charging: boolean = true,
	) => {
		dispatch(globalCharging(item, charging));
	};

	const isCharging = Object.values(stateCharging || {}).includes(true);
