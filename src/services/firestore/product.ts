...
const products = db.collection(
	'Products',
) as firebase.firestore.CollectionReference<IFireProduct>;

const mods = (id, ref?: ProductRef) =>
	ref?.collection('Modifications') ||
	products.doc(id).collection('Modifications');

export const createRef = (): ProductRef => products.doc();

export const createModRef = (productId: string, productRef?: ProductRef) =>
	mods(
		productId,
		productRef,
	).doc() as firebase.firestore.DocumentReference<IProductMod>;
...
export const addProduct = async (
	...
	if (images?.all.length) {
		await new Promise((resolve) => {
			const results = images.all.map((image, index) =>
				uploadImage(docRef.id, `${modRef.id}-${index}`, image),
			);
				...
				complete: async () => {
					product.images.all = await Promise.all(
						results.map(async (res) => ({
							publicURL:
								(await res.imageRef.getDownloadURL()) as string,
			...
			product.relatedProducts.map(async (related) => {
				...
				batch.update(
					...
					relatedData.relatedProducts
						? [...relatedData.relatedProducts, productRelated]
						: [productRelated],
				);
			}),
		);
...
export const getProducts = () =>
	new Observable<QuerySnapshot>((observer) => {
		products.onSnapshot(
			(res: QuerySnapshot) => {
				observer.next(res);
			},
		...
		await Promise.all(
			relateds.map(async (related) => {
				...
				if (relatedData?.relatedProducts?.length) {
					const newRelated = relatedData.relatedProducts?.filter(
						(productRelated) => productRelated.ref.id !== id,
					);
				...
				if (relateds?.length) {
					...
					await Promise.all(
						relateds.map(async (related) => {
							...
							if (relatedData?.relatedProducts?.length) {
								const newRelated =
									relatedData.relatedProducts?.filter(
										(productRelated) =>
											productRelated.ref.id !== id,
									);
								newBatch.update(
									related.ref,
									'relatedProducts',
