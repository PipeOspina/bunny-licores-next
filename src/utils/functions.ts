...
import { IProduct, IRelatedProduct } from '@interfaces/Product';
	...
	switch (errorCode) {
		case FireAuthErrorTypes.INTERNAL_ERROR: {
			return {
				...alert,
				message:
					'Ocurrió un error inesperado al <b>Iniciar Sesión</b>, intentalo nuevamente y si el error persiste comunicate con <b>Pipe</b> :p',
			};
		}
		case FireAuthErrorTypes.POPUP_CLODES: {
			return {
				...alert,
				message:
					'Se cerró el mensaje de <b>Inicio de Sesión</b> antes de tiempo, inténtalo nuevamente sin cerrar la ventana emergente',
			};
		}
		case FireAuthErrorTypes.USER_DISABLED: {
			return {
				...alert,
				message:
					'Estás <b>inhabilitado</b> para entrar a administrar <strong>Bunny Licores</strong>, pídele a <b>Pipe</b> que te habilite nuevamente',
			};
		}
		default: {
			return {
...
export const sortProducts = (
	...
	products
		...
		.sort((productA, productB) => {
			...
			const defaultOrderAsc =
				creationA < creationB ? -1 : creationA > creationB ? 1 : 0;

			const defaultOrderDesc =
				creationA > creationB ? -1 : creationA < creationB ? 1 : 0;

			const defaultOrder =
				order.direction === 'desc' ? defaultOrderAsc : defaultOrderDesc;
		...
		.sort((productA, productB) => {
			...
			const defaultAsc =
				parsedA < parsedB ? -1 : parsedA > parsedB ? 1 : 0;
			...
			switch (order.id) {
				...
				default: {
					return isAsc ? defaultAsc : defaultDesc;
				}
			}
		});
