...
import { Close, Done, PhotoCamera } from '@material-ui/icons';
...
const CreateProduct: FC<Props> = ({ hideButton, products }) => {
	...
	const { setSubscribtion, unsubscribe } =
		useSubscription<IProductSubscriptions>();
	const { setCharging } =
		useCharging<ICreateProductCharging>('createProduct');
			...
			if (!files.find((file) => !file.type.includes('image'))) {
				...
				dispatch(
					addAlert({
						...
						message:
							'Solo puedes subir archivos de tipo <b>imagen</b> para tus productos',
						show: true,
	...
	const handleKeyPressed: KeyboardEventHandler<HTMLDivElement> = (e) => {
		...
		if (type === 'text' && e.key === 'Enter') {
			const nextInput =
				(e.target as any).getAttribute('next-input') || 'Enter';
	...
	return (
		<>
			...
			<Dialog onClose={toggleOpen} open={open} maxWidth="sm" fullWidth>
					...
					<div className={classes.formColumn}>
						...
						>
							<TextField
								...
								onChange={({ target }) =>
									updateProduct('barcode', target.value)
								}
								InputLabelProps={{
									shrink: true,
								}}
								inputProps={{
									...
									'next-input':
										'create-product-dialog-name-input',
								}}
							...
							<TextField
								...
								inputProps={{
									...
									'next-input':
										'create-product-dialog-description-input',
								}}
								error={!!errors.name}
							...
							<div className={classes.formColumn}>
								<TextField
									...
									inputProps={{
										...
										'next-input':
											'create-product-dialog-related-input',
									}}
									multiline
									rowsMax={3}
									rows={3}
										...
										renderInput={(params) => (
											<TextField
												...
												inputProps={{
													...
													'next-input':
														'create-product-dialog-buy-price-input',
												}}
											/>
										)}
										multiple
						...
						>
							<TextField
								...
								onChange={({ target }) => {
									...
									const price = Number.isNaN(value)
										? 0
										: value;
								...
								inputProps={{
									...
									'next-input':
										'create-product-dialog-sell-price-input',
								}}
								InputLabelProps={{
									shrink: true,
								}}
						...
						<FormControlLabel
							control={
								<Checkbox
									checked={haveDeposit}
									onChange={() =>
										setHaveDeposit((current) => !current)
									}
									color="primary"
								/>
							}
							...
							>
								<TextField
									...
									inputProps={{
										...
										'next-input':
											'create-product-dialog-sell-deposit-input',
									}}
									required
								/>
								<TextField
				...
				<DialogActions>
					<Button onClick={toggleOpen}>Cancelar</Button>
					<Button
						color="primary"
						variant="contained"
			...
			<Dialog open={openConfirm} onClose={() => setOpenConfirm(false)}>
				...
				<DialogContent>
					<DialogContentText className={classes.confirm}>
						¿Estás seguro que deseas cerrar la creación del
						producto&nbsp;
						<strong>
