...
const Head = <T extends unknown>(props: Props<T>) => {
	const { columns, checkbox, classes, order, handleSort } = props;
	...
	const handleSortBy = (id: keyof T) => {
		if (order) {
			const orderDesc = order.direction === 'desc' ? 'asc' : 'desc';
			const newOrder: Order<keyof T> = {
				direction: order.id === id ? orderDesc : 'desc',
				id:
					order.id === id && order.direction === 'asc'
						? undefined
						: id,
			};
	...
	return (
		<TableHead className={classes?.root}>
			<TableRow className={classes?.row}>
				{checkbox && (
					<Grow
						in={match || checkbox.rowsSelected !== 0}
						unmountOnExit
					>
						<TableCell
				...
				{checkbox?.header && checkbox?.rowsSelected !== 0 ? (
					...
					columns.map((column) => {
						...
						const key = `${router.pathname.toUpperCase()}-TABLE-HEADER-${id
							.toString()
							.toUpperCase()}`;
						return (
							<TableCell
								...
								align={align || 'left'}
								sortDirection={
									order?.id === id ? order?.direction : false
								}
